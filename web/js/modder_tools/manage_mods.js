// "Mods" management tab — manage the signed-in user's own Mods Hub projects.
// Lists my mods and supports the high-frequency lifecycle (create, edit core
// details, visibility, delete) via the authenticated backend.auth eel calls.
// Heavy studio operations (releases, files, images, collaborators) deep-link to
// the website via "Open in studio".
    ...
    const app = createApp({
        setup() {
            ...
            const setVisibility = async (m, visibility) => {
                ...
                const res = await eel.site_mod_update(m.handle, m.slug, { visibility })();
            ...
            const saveEdit = async () => {
                ...
                // Only the card-backed fields — description etc. are edited in the
                // studio. PATCH leaves any field we don't send untouched.
                const patch = {
                    title,
                ...
                try {
                    const res = await eel.site_mod_update(editForm.handle, editForm.slug, patch)();
            ...
            // The orchestrator dispatches `_shown` right after mount on first open
            // and on every subsequent activation, so load there (no onMounted load
            // → avoids a double fetch on the first open).
            const onShown = () => load();
            ...
            return { t, loading, error, items, busy, createForm, editForm, bannerUrl, openStudio, load, createMod, setVisibility, openEdit, saveEdit, removeMod };
        },
    });
