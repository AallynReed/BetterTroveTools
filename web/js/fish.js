function initFishView() {
    ...
    const app = createApp({
        setup() {
            ...
            const totalPages = computed(() => Math.max(1, Math.ceil(filteredFish.value.length / pageSize.value)));
            const paginatedFish = computed(() => filteredFish.value.slice((currentPage.value - 1) * pageSize.value, currentPage.value * pageSize.value));
            const visibleStart = computed(() => filteredFish.value.length === 0 ? 0 : (currentPage.value - 1) * pageSize.value + 1);
            const visibleEnd = computed(() => filteredFish.value.length === 0 ? 0 : Math.min(currentPage.value * pageSize.value, filteredFish.value.length));
            ...
            return {
                t, isLoading, loadError, fishData, filteredFish, paginatedFish,
