function initMementosView() {
    ...
    if (typeof Vue === 'undefined') {
        ...
        root.innerHTML = `<div class="search-stats" style="color: var(--danger-ink); padding: var(--t-5);">Vue failed to load for Memento Codex.</div>`;
        return;
    }
    ...
    try {
        ...
        root.removeAttribute('v-cloak');
        root.innerHTML = `<div class="search-stats" style="color: var(--danger-ink); padding: var(--t-5);">Failed to initialize Memento Codex: ${String((err && err.message) || err)}</div>`;
    } finally {
