...
def _write_cached_fish(data: dict, manifest: dict) -> None:
    _fish_cache_file().write_text(json.dumps(data, indent=4), encoding="utf-8")
...
def _build_fish_from_game_files(force_refresh: bool = False, game_path_str: str = "") -> tuple[dict, dict, str]:
    ...
    cached_data, cached_manifest = _read_cached_fish()
    ...
    for root in _cache_root_candidates():
        try:
            ...
            _write_cached_fish(data, full_manifest)
            return data, full_manifest, "game-live"
        except Exception as exc:
            last_error = exc
            continue
    raise last_error or RuntimeError("Failed to build fish data from game files.")
