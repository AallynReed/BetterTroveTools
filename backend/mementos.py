...
def _write_cached_mementos(data: dict, manifest: dict) -> None:
    _mementos_cache_file().write_text(codex_cache.compact_dumps(data), encoding="utf-8")
    _mementos_cache_manifest_file().write_text(json.dumps(manifest, indent=2), encoding="utf-8")


def _build_mementos_dataset(game_path: Path) -> tuple[dict, dict]:
...
def _build_mementos_from_game_files(force_refresh: bool = False, game_path_str: str = "") -> tuple[dict, dict, str]:
    game_path = resolve_game_install(game_path_str)
    return codex_cache.resolve_cached_or_build(
        read_cached=_read_cached_mementos,
        is_fresh=_cache_is_fresh,
        is_compatible=_cache_is_compatible,
        build=_build_mementos_dataset,
        write=_write_cached_mementos,
        lock=_BUILD_LOCK,
...
@eel.expose
@standardize_response
def get_mementos_data(force_refresh: bool = False, game_path_str: str = ""):
    try:
        data, manifest, source = _build_mementos_from_game_files(force_refresh=bool(force_refresh), game_path_str=game_path_str)
        cache_file = _mementos_cache_file()
