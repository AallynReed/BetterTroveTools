...
def _write_cached_recipes(data: dict, manifest: dict) -> None:
    _recipes_cache_file().write_text(codex_cache.compact_dumps(data), encoding="utf-8")
    _recipes_cache_manifest_file().write_text(json.dumps(manifest, indent=2), encoding="utf-8")


def _build_recipes_dataset(game_path: Path) -> tuple[dict, dict]:
...
def _build_recipes_from_game_files(force_refresh: bool = False, game_path_str: str = "") -> tuple[dict, dict, str]:
    ...
    return codex_cache.resolve_cached_or_build(
        read_cached=_read_cached_recipes,
        is_fresh=_cache_is_fresh,
        is_compatible=_cache_is_compatible,
        build=_build_recipes_dataset,
        write=_write_cached_recipes,
        lock=_BUILD_LOCK,
