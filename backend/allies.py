...
def _write_cached_allies(data: dict, manifest: dict) -> None:
    _allies_cache_file().write_text(codex_cache.compact_dumps(data), encoding="utf-8")
    _allies_cache_manifest_file().write_text(json.dumps(manifest, indent=2), encoding="utf-8")


def _build_allies_dataset(game_path: Path) -> tuple[dict, dict]:
    extract_dir = _cache_root() / "allies_runtime_cache"
    data, manifest = asyncio.run(
        build_allies_dataset(game_path=game_path, extract_dir=extract_dir, locale="en")
    )
...
def _build_allies_from_game_files(force_refresh: bool = False, game_path_str: str = "") -> tuple[dict, dict, str]:
    ...
    return codex_cache.resolve_cached_or_build(
        read_cached=_read_cached_allies,
        is_fresh=_cache_is_fresh,
        is_compatible=_cache_is_compatible,
        build=_build_allies_dataset,
        write=_write_cached_allies,
        lock=_BUILD_LOCK,
