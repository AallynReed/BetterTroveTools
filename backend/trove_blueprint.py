...
from backend import trove_blueprint_codec as codec
from backend import trove_blueprint_maps as maps
from backend.trove_blueprint_codec import (
    DEFAULT_TYPE,
    DEFAULT_W,
    DecodedBlueprint,
    is_empty_blueprint,
...
def _render_resolver(game_path):
    if not game_path:
        return None
    try:
        from backend.trove_block_mapping import load_deco_resolver
...
def blueprint_to_package(blueprint_path: str | Path, game_path: str | Path | None = None) -> dict:
    ...
    if game_path:
        ...
        try:
            ...
            exact_colors = None
        try:
            from backend.trove_block_mapping import load_block_resolver
