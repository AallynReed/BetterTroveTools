...
# 1. Initialize the engine globally safely
gem_engine = None
try:
    # Adjust base_path if your Python script runs from a different root directory!
...
# 2. Expose the data fetchers with safe fallbacks
@eel.expose
def get_trove_classes():
    if not gem_engine:
        return []
    try:
        # FIX: Use cls.name.value so Pydantic parses the Enum correctly during calculation
...
# 3. Expose the calculator safely
@eel.expose
...
def parse_star_chart_code(base64_code):
    try:
        # Returns {"stats": {"Light": {"flat": 150, "pct": 0}, ...}, "abilities": [...]}
