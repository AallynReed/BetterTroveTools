...
def _sync_cache(exe_path: Path, content: bytes, offset: int) -> None:
    ...
    # Persist the decoded FPS keyed by the file's mtime so a future run (or the
    # first get_settings after launch) can return it WITHOUT reading the whole
    # exe. Any content change -- a patch or a game update -- bumps st_mtime_ns,
    # which invalidates this and forces a real read.
    try:
        rec["fps"] = _fps_from_value(content[offset:offset + 8])
        rec["fps_mtime_ns"] = exe_path.stat().st_mtime_ns
    except OSError:
        pass
...
def get_current_fps(exe_path: Path) -> int | None:
    ...
    try:
        ...
        # Disk fast path: if a previous run recorded the FPS for this exact build
        # (same size + mtime), return it without reading the ~21 MB file. This is
        # what keeps startup and menu switching cheap on an unchanged install.
        rec = _read_index().get(_install_key(exe_path))
        if (
            rec
            and rec.get("size") == st.st_size
            and rec.get("fps_mtime_ns") == st.st_mtime_ns
            and isinstance(rec.get("fps"), int)
        ):
            _MEMO[memo_key] = rec["fps"]
            return rec["fps"]
